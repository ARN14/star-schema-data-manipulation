...
def populate_new_database():
    ...
    for staff in dim_staff:
        conn.run("INSERT INTO dim_staff (first_name, last_name, department_id) VALUES (:first, :last, :id)", first=staff[0], last=staff[1], id=staff[2])

    new_staff = conn.run("SELECT * FROM dim_staff;")
    fact_sales = format_sales(new_stock, new_staff, sales_data)

    for sale in fact_sales:
        conn.run("INSERT INTO fact_sales (item_id, salesperson, price, quantity, created_at) VALUES (:id, :salesperson, :price, :quantity, :created)", id=sale[0], salesperson=sale[1], price=sale[2], quantity=sale[3], created=sale[4])
